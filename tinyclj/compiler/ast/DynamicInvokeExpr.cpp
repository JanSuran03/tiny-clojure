...
EmitResult DynamicInvokeExpr::emitIR(CodegenContext &ctx) const {
...
DynamicInvokeExpr::DynamicInvokeExpr(AExpr invokeTarget,
                                     std::vector<AExpr> invokeArgs)
        : InvokeExpr(std::move(invokeArgs)),
          m_InvokeTarget(std::move(invokeTarget)) {}

const Object *DynamicInvokeExpr::evalInvoke(const std::vector<const Object *> &evaled_args) const {
    ...
    // bruh what is this const cast
    return callFn(evaled_target, evaled_args.size(), const_cast<const Object **>(evaled_args.data()));
}
